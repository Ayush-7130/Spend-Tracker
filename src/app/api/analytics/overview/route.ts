...
export async function GET() {
  try {
    ...
    const currentMonthExpenses = await db.collection('expenses').aggregate([
      {
        $match: {
          date: {
    ...
    const lastMonthExpenses = await db.collection('expenses').aggregate([
      {
        $match: {
          date: {
            $gte: lastMonthStart,
    ...
    const topCategories = await db.collection('expenses').aggregate([
      ...
      },
      {
        $project: {
          ...
          percentage: {
            $multiply: [
              { $divide: ['$amount', currentMonthTotal > 0 ? currentMonthTotal : 1] },
              100
            ]
          }
        }
    ...
    const highestExpenseDay = await db.collection('expenses').aggregate([
      {
        $match: {
          date: {
            $gte: currentMonthStart,
                  ...
                  $arrayElemAt: [
                    {
                      $filter: {
                        input: '$expenses',
                        cond: { $eq: ['$$this.amount', { $max: '$expenses.amount' }] }
                      }
                    },
                    0
                  ]
    ...
    const overviewData = {
      ...
      highestExpenseDay: highestExpenseDay[0] ? {
        ...
        description: highestExpenseDay[0].description || 'Multiple expenses'
      } : {
