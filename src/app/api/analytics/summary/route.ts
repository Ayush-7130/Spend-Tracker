...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const expensesByPerson = await db.collection('expenses').aggregate([
      {
        $group: {
          _id: '$paidBy',
    ...
    const splitExpenses = await db.collection('expenses').find({
      isSplit: true
    }).toArray();
    ...
    const thisMonthExpenses = await db.collection('expenses').aggregate([
      {
        $match: {
          date: {
            $gte: startOfMonth,
