...
export async function DELETE(
  ...
  try {
    ...
    const expensesUsingCategory = await db.collection('expenses').countDocuments({
      category: id
    });
    ...
    const result = await db.collection<Category>('categories').deleteOne({ _id: id });    if (result.deletedCount === 0) {
      return NextResponse.json(
        { success: false, error: 'Category not found' },
        { status: 404 }
      );
