...
export async function DELETE(
  ...
  try {
    ...
    const settlement = await db.collection('settlements').findOne({ _id: new ObjectId(id) });
    ...
    try {
      ...
      if (currentUser) {
        const otherUserName = settlement.fromUser === currentUser.name ? settlement.toUser : settlement.fromUser;
        ...
        if (otherUser && otherUser._id.toString() !== user.userId) {
          await notificationService.sendNotification(
            otherUser._id.toString(),
            {
              ...
              entityName: settlement.description || `Settlement from ${settlement.fromUser} to ${settlement.toUser}`,
        ...
        if (otherUser && otherUser._id.toString() !== user.userId) {
          await notificationService.sendNotification(
            otherUser._id.toString(),
            {
              ...
              actorName: currentUser.name,
              entityName: description || `Settlement from ${fromUser} to ${toUser}`,
              amount: parseFloat(amount)
            }
          );
        }
