...
const NotificationBell: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      window.dispatchEvent(new CustomEvent('notificationPanelOpen', { detail: true }));
    } else {
      window.dispatchEvent(new CustomEvent('notificationPanelOpen', { detail: false }));
    }
    ...
    <div className="position-relative" ref={bellRef}>
      <div
        ...
        style={{ 
          ...
          borderRadius: '6px',
          background: isOpen ? 'var(--notification-bell-active)' : 'var(--notification-bell-bg)',
      ...
      >
        <i className={`bi bi-bell-fill ${theme === 'dark' ? 'text-white' : 'text-dark'}`} style={{ padding: '0 0.2rem' }}></i>
        {unreadCount > 0 && (
          <span
        ...
        >
          <div style={{
            ...
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mb-0 fw-bold" style={{ color: 'var(--notification-panel-text)' }}>Notifications</h6>
              <div className="d-flex align-items-center gap-2">
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = 'var(--notification-panel-item-hover)';
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = 'transparent';
                  }}
                >
                  <i className="bi bi-x" style={{ fontSize: '18px', lineHeight: '1' }}></i>
                </button>
              </div>
            </div>
            {notifications.length > 0 && (
              <small style={{ color: 'var(--notification-panel-text-secondary)' }}>
                {unreadCount > 0 ? `${unreadCount} unread of ${notifications.length}` : `${notifications.length} notifications`}
              </small>
            )}
          </div>
          ...
          <div className="notification-content" style={{
            ...
            {error && (
              <div className="alert alert-danger mx-2 mt-2 mb-2" role="alert" style={{
            ...
            {loading ? (
              ...
              <div className="text-center p-4">
                <div className="mb-2" style={{ color: 'var(--notification-panel-text-secondary)' }}>
                  <i className="bi bi-bell-slash fa-2x"></i>
                </div>
                <div style={{ color: 'var(--notification-panel-text-secondary)' }}>
                ...
                {notifications.map((notification) => (
                  <div
                    ...
                    style={{
                      padding: '10px 12px',
                      borderBottom: '1px solid var(--notification-panel-header-border)',
                      background: !notification.read ? 'var(--notification-panel-unread-bg)' : 'transparent',
                      cursor: 'pointer',
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = !notification.read ? 'var(--notification-panel-unread-bg)' : 'transparent';
                    }}
                  >
                    <div className="d-flex align-items-start">
                      ...
                      <div className="flex-grow-1">
                        <div className="d-flex justify-content-between align-items-start">
                          <p className={`mb-1 ${!notification.read ? 'fw-bold' : 'fw-normal'}`} style={{
                            fontSize: '0.8rem',
                            color: 'var(--notification-panel-text)',
                            lineHeight: '1.2'
                          }}>
