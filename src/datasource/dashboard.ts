...
export class DashboardDataSource {
  ...
  static async getQuickStats(
    user?: "all" | "saket" | "ayush"
  ): Promise<DashboardData["quickStats"]> {
    return withRetry(() =>
      api.get<DashboardData["quickStats"]>("/dashboard/quick-stats", {
        user: user || "all",
      })
    );
  }
   ...
   */
  static async getWidgetConfig(): Promise<
    Array<{
      id: string;
  ...
  > {
    return withRetry(() => api.get("/dashboard/widgets"));
  }
  ...
  static async updateWidgetConfig(
    ...
    return withRetry(() => api.put("/dashboard/widgets", { widgets: config }));
  }
}
