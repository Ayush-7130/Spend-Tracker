...
export default function Table<T = any>({
  ...
  const [currentSort, setCurrentSort] = useState<TableSort | null>(
  ...
  const getNestedValue = useCallback(
    (obj: any, path: string | keyof T): any => {
      return String(path)
        .split(".")
        .reduce((o, p) => o?.[p], obj);
    },
  ...
  const paginatedData = useMemo(() => {
    ...
    if (
      ...
      config.pagination.total > config.data.length
    ) {
  ...
  const actualPages = config.pagination
    ? Math.ceil(actualTotal / config.pagination.limit)
    : 1;
    ...
    (column: TableColumn<T>, row: T, index: number) => {
      if (column.render) {
        return column.render(
          getNestedValue(row, column.accessor || column.key),
          row,
          index
        );
      }
  ...
  return (
    ...
    >
        ...
        >
              ...
              {config.selectable && (
                <th>
                  <input
                    type="checkbox"
                    checked={
                      config.selection?.selectedRows.length ===
                ...
                >
                  ...
                  {column.sortable !== false &&
                    ...
                    currentSort?.column === column.key && (
                      <i
                        className={`bi ${currentSort.direction === "asc" ? "bi-chevron-up" : "bi-chevron-down"} ms-1`}
                      ></i>
                    )}
                </th>
              ))}
          ...
          <tbody>
            {paginatedData.map((row, index) => {
              const rowId = config.keyExtractor(row);
              const isSelected =
                config.selection?.selectedRows.some(
                  (r) => config.keyExtractor(r) === rowId
                ) || false;
      ...
      {/* Mobile Card View */}
      <div className="table-mobile-cards d-md-none">
        {paginatedData.map((row, index) => {
          const rowId = config.keyExtractor(row);
          const isSelected =
            config.selection?.selectedRows.some(
            ...
            >
              {config.selectable && (
                <div className="table-mobile-card-row">
                  <div className="table-mobile-card-label">
                    <input
                      type="checkbox"
                      checked={isSelected}
              ...
              {config.columns.map((column) => (
                <div key={column.key} className="table-mobile-card-row">
                  <div className="table-mobile-card-label">{column.header}</div>
              ...
              {config.actions && config.actions.length > 0 && (
                <div className="table-mobile-card-actions">
                  {config.actions.map((action) => {
        ...
        <div className="table-pagination-wrapper p-3">
          ...
          <nav className="table-pagination-nav">
            <ul className="pagination pagination-sm mb-0">
              ...
              {/* Page numbers */}
              {config.pagination &&
                Array.from({ length: actualPages }, (_, i) => i + 1)
                  .filter(
                    (page) =>
                      config.pagination &&
              ...
              <li
                className={`page-item ${currentPage >= actualPages ? "disabled" : ""}`}
              >
                <button
                  onClick={() => handlePageChange(currentPage + 1)}
